...
(module).exports = function () {
    ...
    return model_1.Model.loadRepoConfig().then(function (repoConfig) {
        return model_1.Model.loadGlobalConfig().then(function (globalConfig) {
            ...
            return model_1.Model.writeFile(templateName, template(repoConfig));
        });
    });
};
/**
...
function loadTemplate(teamUrl, configRepo, branch, templateName) {
    return loadLocalTemplate(teamUrl, configRepo, branch, templateName).then(function (template) {
        if (template) {
            return template;
        }
...
function loadLocalTemplate(teamUrl, configRepo, branch, templateName) {
    var templatePath = model_1.BUILD_HELPER_CACHE + 'templates/' + templateName;
    ...
    return new Promise(function (resolve, reject) {
        fs.exists(localPath, function (exists) {
            if (exists) {
                ...
                model_1.Model.readFile(localPath, 'utf8').then(resolve);
            }
            else {
                resolve(null);
            }
...
function loadRemoteTemplate(teamUrl, configRepo, branch, templateName) {
    var templatePath = model_1.BUILD_HELPER_CACHE + 'templates/' + templateName;
